...
const char_T *rt_UpdateTXXFYLogVars(RTWLogInfo *li, time_T *tPtr, boolean_T updateTXY)
{
    ...
    if (matrixFormat) {                                      /* MATRIX_FORMAT */
        ...
        if (logInfo->x != NULL || logInfo->xFinal != NULL) {
            ...
            if (logInfo->x != NULL && updateTXY) {
                rt_UpdateLogVarWithDiscontiguousData(logInfo->x, segAddr,
                                                     segLengths, nSegments, preprocessingPtrs);
            }
            ...
            for (i = 0, yIdx = 0; i < ny; i++) {
                if (data[i] != NULL) {
                    ...
                    */
                    ...
                    if (preprocessingPtr != NULL) {
                        void* curData = rt_getTempMemory(var[yIdx], yInfo, i);
                        preprocessingPtr(curData, data[i]);
                        rt_UpdateLogVar(var[yIdx++], curData, false);
                        free(curData);
                    }
                    else {
        ...
        if (logInfo->x != NULL && updateTXY) {
            ...
            /* signals */
            for (i = 0; i < nsig; i++) {
                RTWPreprocessingFcnPtr preprocessingPtr = xInfo->preprocessingPtrs[i];
                if (preprocessingPtr != NULL) {
                    void* curData = rt_getTempMemory(val, xInfo, i);
                    preprocessingPtr(curData, data[i]);
                    rt_UpdateLogVar(val, curData, false);
                    free(curData);
                }
                else {
